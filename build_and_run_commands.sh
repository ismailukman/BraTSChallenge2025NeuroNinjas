...
PROJECT_ID="syn68827053" #  (**) Change to your Project Synapse Project ID (same as the one in the val stage)
IMAGE_NAME="brats-ssa-sparkNNC" # (**) The name you would like to give your Docker image 
TAG="NNCV1"  # Tag for the Docker image (e.g., "latest", "v1", etc.)
DOCKERFILE_DIR="." # Directory containing the Dockerfile

INPUT_DIR="$(pwd)/data/example_input/BraTS2023_TrainingData" # Absolute path to the input directory 
OUTPUT_DIR="$(pwd)/data/example_output/BraTS2025_predictions_3D" # Absolute path to the output directory

SYNAPSE_USERNAME="ykatuka" # (**) Replace with your Synapse username 
